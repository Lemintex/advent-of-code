...
func CanBeSolved(cal calibration, arrayIndex, num int) bool {
	...
	join := JoinNums(num, cal.operands[arrayIndex])
	return CanBeSolved(cal, arrayIndex + 1, add) || CanBeSolved(cal, arrayIndex + 1, mul) || CanBeSolved(cal, arrayIndex + 1, join)
}

func JoinNums(a, b int) int {
	strA := strconv.Itoa(a)
	strB := strconv.Itoa(b)
	str := strA + strB
